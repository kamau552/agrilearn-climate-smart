...
export default function Sidebar({ isOpen, onToggle }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed top-0 left-0 h-dvh bg-linear-to-br from-green-800 to-green-900 text-white transition-all duration-300 z-50 shadow-xl flex flex-col
          ${isOpen ? "w-72" : "w-20"}
          ${
            isMobile
              ? isOpen
              ...
              {isOpen ? (
                <>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-linear-to-br from-white to-green-50 rounded-xl flex items-center justify-center shrink-0 shadow-md">
                      <Image
                ...
                <div className="w-12 h-12 bg-linear-to-br from-white to-green-50 rounded-xl flex items-center justify-center mx-auto shadow-md">
                  <Image
                    src="/assets/images/Logo.png"
                    alt="AgriLearn"
                    width={28}
            ...
            {mainMenuItems.map(({ name, icon, path, badge, badgeColor }) => {
              ...
              return (
                <Link
                  ...
                  `}
                >
                  <div className={`text-xl shrink-0 ${isActive ? "scale-110" : ""}`}>
                    {icon}
                  </div>
                  ...
                  {isOpen && (
                    <div className="flex justify-between items-center w-full min-w-0">
                      ...
                      {badge && (
                        <span
                          className={`${badgeColor} text-xs text-white px-2 py-0.5 rounded-full shrink-0 ml-2`}
                        >
                          {badge}
                        </span>
                      )}
